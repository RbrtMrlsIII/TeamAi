...
if (process.argv[2] === 'recover') {
  ...
  try {
    ...
    if (result.result?.text !== expectedText) throw new Error(`Recovered result mismatch: expected ${expectedText}`);
    process.stdout.write(JSON.stringify({ recovered: true, taskId: result.taskId, eventId: result.eventId, text: result.result.text }));
    process.exit(0);
  } catch (error) {
    process.stderr.write(error instanceof Error ? error.message : String(error));
    process.exit(1);
...
if (recovery.exitCode !== 0) throw new Error(`Restart/recovery probe failed: ${recovery.stderr}`);
console.log(JSON.stringify({ phase: 'restart-recovery', ...JSON.parse(recovery.stdout) }));
await cleanup(winner.leaseId);
console.log(JSON.stringify({ phase: 'complete', status: 'live-firestore-contention-and-recovery-pass', cleanup: 'done' }));

async function writeTask() {
  const token = await accessToken();
  const url = documentUrl(taskPath);
  const response = await fetch(`${ROOT}/projects/${encodeURIComponent(projectId)}/databases/(default)/documents:commit`, {
    method: 'POST',
    headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
    body: JSON.stringify({
      writes: [{ update: { name: url, fields: { uid: { stringValue: uid }, projectId: { stringValue: testProjectId }, status: { stringValue: 'ready' }, createdAt: { timestampValue: new Date().toISOString() } } }, currentDocument: { exists: false } }],
    }),
  });
...
function spawnWorker(actorId) {
  return spawn(process.execPath, [script, 'worker'], { env: { ...process.env, TEAMAI_LIVE_ACTOR: actorId }, stdio: ['ignore', 'pipe', 'pipe'] });
}

function spawnRecovery(eventId, expectedText) {
  return new Promise((resolve) => {
    const child = spawn(process.execPath, [script, 'recover'], { env: { ...process.env, TEAMAI_LIVE_RESULT_EVENT_ID: eventId, TEAMAI_LIVE_EXPECTED_RESULT: expectedText }, stdio: ['ignore', 'pipe', 'pipe'] });
    let stdout = '';
    let stderr = '';
...
function parseWorkerResult(child) {
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (exitCode) => {
      ...
      try { resolve(JSON.parse(stdout)); } catch { reject(new Error(`Worker emitted invalid JSON: ${stdout}`)); }
    });
  });
}
...
async function cleanup(leaseId) {
  ...
  const names = [taskPath, `${taskPath}/leases/${leaseId}`, `${taskPath}/execution-results/${resultEventId}`];
  const response = await fetch(`${base}:commit`, {
    method: 'POST',
    headers: { authorization: `Bearer ${token}`, 'content-type': 'application/json' },
    body: JSON.stringify({ writes: names.map((path) => ({ delete: `${base}/${path.split('/').map((segment) => encodeURIComponent(segment)).join('/')}` })) }),
  });
  if (!response.ok) throw new Error(`Live test cleanup failed: ${response.status}`);
}

function documentUrl(path) {
  return `${ROOT}/projects/${encodeURIComponent(projectId)}/databases/(default)/documents/${path.split('/').map((segment) => encodeURIComponent(segment)).join('/')}`;
}
...
function loadServiceAccount() {
  ...
  if (parsed.project_id !== projectId) throw new Error('Firebase project identity mismatch');
  return parsed;
}
