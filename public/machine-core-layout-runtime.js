...
const innerProfiles=Array.from({length:8},(_,seatIndex)=>({branchId:`BRANCH-SEAT-${String(seatIndex+1).padStart(2,'0')}`,seatIndex,angle:TAU*seatIndex/8,height:[.62,.76,.68,.83,.70,.88,.66,.80][seatIndex]}));
const outerProfiles=[
 {branchId:'BRANCH-OUTER-ALPHA',angle:TAU*.25/4,height:.94,silhouette:'fin'},
 {branchId:'BRANCH-OUTER-BETA',angle:TAU*1.25/4,height:1.08,silhouette:'arc'},
 {branchId:'BRANCH-OUTER-GAMMA',angle:TAU*2.25/4,height:.98,silhouette:'diamond'},
