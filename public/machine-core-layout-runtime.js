import { bindSeatSemantic, SEAT_SEMANTIC_REGISTRY } from './machine-core-semantic-map.js';
...
const outerAngle=(count,outerIndex)=>{const quadrant=TAU*outerIndex/OUTER_COUNT,step=TAU/count,nearestSeatIndex=Math.floor(quadrant/step);return(nearestSeatIndex+.5)*step;};
export function createBranchConnectionCore({seatCount=DEFAULT_SEAT_COUNT,expanded=false,expansionAmount=null}={}){
 const count=Math.min(MAX_SEAT_COUNT,Math.max(MIN_SEAT_COUNT,Math.floor(Number(seatCount)||DEFAULT_SEAT_COUNT))),amount=expansionAmount==null?(expanded?1:0):clamp01(expansionAmount),innerRadius=4.05+.5*amount,podPortRadius=.92+.10*amount,outerRadius=6.45+.70*amount;
 ...
 hub.uiSurface=uiSurface(hub,hub.uiStyle,.72);hub.semanticId=null;hub.semanticBoundary='presentation-only';hub.camera=cameraForPart(hub,Math.PI/2);
 const inner=Array.from({length:count},(_,seatIndex)=>{const angle=TAU*seatIndex/count,level=seatLevel(seatIndex),center=polar(innerRadius,angle,level),d=dims.pod,branchId=`BRANCH-SEAT-${String(seatIndex+1).padStart(2,'0')}`,semantic=bindSeatSemantic({seatIndex}),part={id:`branch-seat-${String(seatIndex+1).padStart(2,'0')}`,branchId,seatIndex,kind:'inner-pod',level,center,dimensions:{...d},silhouette:'pod',seam:d.seam,uiStyle:'seat-configuration',port:polar(podPortRadius,angle,level),...semantic};part.uiSurface=uiSurface(part,part.uiStyle);part.camera=cameraForPart(part,angle);return part});
 const outer=outerProfiles.map((p,outerIndex)=>{const angle=outerAngle(count,outerIndex),center=polar(outerRadius,angle,p.height),d=dims[p.silhouette],part={id:p.branchId.toLowerCase(),branchId:p.branchId,seatIndex:null,kind:'outer-housing',level:p.height,center,dimensions:{...d},silhouette:p.silhouette,seam:d.seam,uiStyle:p.uiStyle,port:polar(1.12,angle,p.height)};part.uiSurface=uiSurface(part,p.uiStyle,p.uiScale);part.semanticId=null;part.semanticBoundary='presentation-only';part.camera=cameraForPart(part,angle);return part});
 const parts=[hub,...inner,...outer],byBranch=new Map(parts.map(p=>[p.branchId,p])),connections=[];
 for(const pod of inner)connections.push({id:`${pod.branchId}:HUB`,sourceBranchId:'HUB-CORE',targetBranchId:pod.branchId,sourcePort:hub.port,targetPort:pod.port,kind:'inner-spoke',semanticRelation:'tree-membership',semanticSourceId:null,semanticTargetId:pod.semanticId,semanticTargetKey:pod.semanticKey,route:[hub.port,{x:pod.center.x*.52,y:Math.max(hub.level,pod.level)+.22,z:pod.center.z*.52},pod.port]});
 ...
 return Object.freeze({seatCount:count,expansionAmount:amount,semanticTreeId:'TREE-HERO-SEAT',semanticRegistry:SEAT_SEMANTIC_REGISTRY,hub,parts:Object.freeze(parts),connections:Object.freeze(connections),cameras:Object.freeze(parts.map(p=>p.camera)),byBranch});
}
