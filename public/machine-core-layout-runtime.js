...
export function createBranchConnectionCore({seatCount=DEFAULT_SEAT_COUNT,expanded=false,expansionAmount=null}={}){
 ...
 const amount=expansionAmount==null?(expanded?1:0):clamp01(expansionAmount),innerRadius=4.05+(.5*amount),podPortRadius=.92+(.10*amount),outerRadius=6.45+(.70*amount);
