# TeamAi Skills — README

Skills are direct operational instructions for bounded classes of work. They are not product authority, permission, or a replacement for `PRODUCT_LAW.md`, `MASTERPLAN.md`, or `POLICY.md`.

## Read this first

`docs/SKILL_WIRING.md` is the TeamAi wiring map that connects canonical concepts and Masterplan checklist items to skill paths, tools, verification, and evidence.

`skills/**/SKILL.md` contains the direct procedure for a specific work class.

## Skill contract

Every skill should state:

`WHEN TO USE → INPUT → AUTHORITY → ACTION → DO NOT → PASS → EVIDENCE → SEE ALSO`

The `SEE ALSO` section should point to deeper canonical context when the skill needs it. The skill itself must still be direct enough to execute without rediscovering the entire repository.

## Permission boundary

A skill cannot grant permission to change Product Law, canonical state, protected history, security boundaries, entitlements, or external accounts. User approval, Policy gates, and service authorization remain binding.

## Growth rule

Create or extend a skill when a recurring checklist/execution pattern needs a reusable procedure. Do not create empty taxonomy folders merely for appearance. Skill families expand from actual consuming-project needs and verified experience.
