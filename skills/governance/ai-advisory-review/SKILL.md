...
There is no inter-stage barrier or pre-sequence timer. The first eligible non-draft lifecycle event that passes substantive exact-head validation writes a durable claim for that exact head and fans out five reusable reviewer jobs concurrently with fail-fast disabled. Each job targets openrouter/free, receives one distinct credential alias, receives the original triggering head, and independently revalidates that head immediately before model invocation. Requests use `response_format.type=json_schema`, `provider.require_parameters=true`, `stream=false`, and the `response-healing` plugin.
