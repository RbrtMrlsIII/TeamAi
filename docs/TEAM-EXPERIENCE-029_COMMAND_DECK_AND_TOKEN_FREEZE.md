...
Every planned composition records:

- purpose: what the user is doing;
- what must be visible;
- primary action;
- elevation peak;
- primitives consumed: Shell, Panels, Cards, Controls, Navigation;
- 029 concepts that must not disappear on compact viewports.
...
The Command Deck is the first inhabited skeleton because it already exercises the 029 concepts that the visual system must carry:
...
Planning vs Working is a product stage inside E3. It is not a second theme and must not fork the skeleton.
