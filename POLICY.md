...
The durable sequence-claim marker is the quota boundary for the exact PR head. A second automatic sequence is forbidden for the same exact head; a later corrected PR head may establish one new sequence after the substantive exact-head validation gate passes. Prior claims and outcomes remain immutable evidence. Draft PRs consume no automatic model calls. Each slot revalidates the original triggering head before invocation and fails closed on a head change.
