...
The advisory reviewer uses a two-layer token policy: a 30,000-token maximum completion ceiling for resilience, plus low reasoning effort and excluded reasoning output to control unnecessary generation. This is a ceiling, not a target. The visible review contract is capped to three concise items per section at 400 characters each, and packet selection is authority-first. Provider HTTP 200, generation-limit truncation, or unavailable routed-model provenance are terminal evidence states, not merge blockers; transport success and review publication are reported separately.
