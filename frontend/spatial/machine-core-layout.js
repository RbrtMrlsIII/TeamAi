...
export function createBranchConnectionCore({ seatCount = DEFAULT_SEAT_COUNT, expanded = false } = {}) {
  ...
  const inner = Array.from({ length: count }, (_, seatIndex) => {
    const angle = TAU * seatIndex / count;
  ...
  const outer = outerProfiles.map((profile) => {
    ...
    return {
      ...
      port: polar(1.12, angle, profile.height),
      camera: makeCamera(profile.branchId, polar(10.2, angle, profile.height + 4.1), center),
    };
  });
  ...
  const connections = [];
  for (const pod of inner) connections.push({
