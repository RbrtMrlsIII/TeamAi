...
function dimensions(value, fallback = { x: 1, y: 1, z: 1 }) {
  ...
  return { x: Math.max(0.001, Number(source.x) || fallback.x), y: Math.max(0.001, Number(source.y) || fallback.y), z: Math.max(0.001, Number(source.z) || fallback.z) };
}
export function makeMachinePart({ id, semanticId, kind = 'division', center, dimensions: size, port = null, active = false } = {}) {
  if (!id || !semanticId) throw new Error('machine part requires id and semanticId');
  return { id, semanticId, kind, center: finitePoint(center), dimensions: dimensions(size), port: port ? finitePoint(port) : null, active: Boolean(active) };
}
export function deriveMachineSubject(parts, padding = 0.12) {
  ...
  const bounds = candidates.reduce((acc, part) => {
    ...
    acc.minX = Math.min(acc.minX, part.center.x - half.x); acc.minY = Math.min(acc.minY, part.center.y - half.y); acc.minZ = Math.min(acc.minZ, part.center.z - half.z);
    acc.maxX = Math.max(acc.maxX, part.center.x + half.x); acc.maxY = Math.max(acc.maxY, part.center.y + half.y); acc.maxZ = Math.max(acc.maxZ, part.center.z + half.z); return acc;
...
export function resolveMachineCamera({ cameraId = 'HERO_WIDE', subject = null, viewport = { width: 1, height: 1 }, distance = 8 } = {}) {
  ...
  const spanX = subject ? subject.max.x - subject.min.x : 1; const spanZ = subject ? subject.max.z - subject.min.z : 1; const fit = Math.max(1, Math.hypot(spanX, spanZ)); const scale = Math.min(width, height) / (fit * 3.4);
  return Object.freeze({ cameraId: String(cameraId), target, distance: Math.max(0.1, Number(distance) || 8), scale, source: subject ? 'semantic-subject' : 'camera-configuration' });
}
export function projectMachinePoint(point, camera, viewport) {
  const p = finitePoint(point); const target = finitePoint(camera?.target); const scale = Number(camera?.scale) || 1; const width = Math.max(1, Number(viewport?.width) || 1); const height = Math.max(1, Number(viewport?.height) || 1);
