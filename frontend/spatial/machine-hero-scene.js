/**
 * Machine Hero scene model and deterministic projector.
 *
 * This module is deliberately renderer-neutral: semantic state owns geometry,
 * geometry owns subject footprint, and the camera consumes that footprint.
 */
...
export function makeMachinePart({
  id,
  semanticId,
  kind = 'division',
  center,
...
export function createMachineTransition({
  ...
  if (!source?.semanticId || !target?.semanticId) {
    throw new Error('machine transition requires source and target semantics');
  }

  const sourcePart = makeMachinePart({ ...source, active: true });
  const targetPart = makeMachinePart(target);
  const transitionParts = [sourcePart, targetPart];
  const subject = deriveMachineSubject(transitionParts);
  ...
  return Object.freeze({
    ...
    expansion: {
      sourceAmount: clamp(expansion.sourceAmount, 0, 1),
      targetAmount: clamp(expansion.targetAmount, 0, 1),
    },
...
export function resolveMachineCamera({
  cameraId = 'HERO_WIDE',
  subject = null,
