...
- [x] Additional reviewer aliases are registered: `dots3-secondary` → `OPENROUTER_API_KEY_GWEN`, `dots3-primary` → `OPENROUTER_API_KEY_DEEPSEEK`, `ling` → `OPENROUTER_API_KEY_OPENAI`, and `poolside` → `OPENROUTER_API_KEY_POOLSIDE`. Secret aliases remain independent of reviewer/model identity.
...
- [ ] Automatic advisory review sequence is runtime-verified as `Nemotron → 2 minutes 30 seconds → Ling 3.0 Flash + Poolside → 2 minutes 30 seconds → Dots3-Note Preview (primary) + Dots3-Note Preview (secondary)` on the first eligible non-draft PR lifecycle event.
...
- [ ] Ordered multi-model orchestration is runtime-verified without weakening or bypassing the existing reviewer gate.
