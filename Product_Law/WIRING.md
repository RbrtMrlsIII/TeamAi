...
The requested route is openrouter/free. Five distinct OpenRouter API key credentials are assigned one per slot; credential aliases are infrastructure identifiers only and do not create Product Law identities. The actual routed model/provider returned by OpenRouter is runtime evidence and is recorded per successful slot. This operational routing does not create reviewer/model/provider identities. review-readiness remains the separate human promotion/authorization gate.
