...
The current configured reviewer aliases are `nemotron` → `OPENROUTER_API_KEY`, `ling` → `OPENROUTER_API_KEY_OPENAI`, `poolside` → `OPENROUTER_API_KEY_POOLSIDE`, `dots3-primary` → `OPENROUTER_API_KEY_DEEPSEEK`, and `dots3-secondary` → `OPENROUTER_API_KEY_GWEN`. These secret aliases are verification/runtime handles only. They are intentionally decoupled from reviewer/model identity and are not new Product Law identities.
