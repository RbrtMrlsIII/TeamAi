...
- #384 is merged and established the current advisory control-plane shape: five parallel `openrouter/free` slots, five distinct OpenRouter credential aliases (one per slot), no inter-slot timers or cohort barriers, exact-head rechecks, explicit terminal slot outcomes, and actual routed-model/provider provenance on successful advisory posts.
- #369 remains open as historical execution evidence for the superseded #368 topology and is not current reviewer orchestration authority.
...
- Current PR: **#388**, Draft, exact head **`a8198ea8fda5dc948dcc817cb484c7eba312d4db`**, based directly on current `main` **`6dc96ff929d1ea874afd3dc44cad9f95fa8096c8`**.
...
- This control-plane reconstruction is being carried by fresh replacement PR #388 because #386's automatic sequence claim was already consumed. Keep #386 Draft and do not solicit human review until the replacement vehicle's automatic five-key sequence completes cleanly.
...
Manual later-head review uses /openrouter-free or /free-1 through /free-5. The former Nemotron-named workflow and Skill are archived; the active manual path uses the generalized OpenRouter Free Router slots without model-specific approval. Model verdicts remain advisory and cannot create Product Law authority, merge authority, acceptance, or human review authorization.
