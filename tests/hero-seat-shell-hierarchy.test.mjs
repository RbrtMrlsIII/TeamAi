...
test('Seat shell v1 part IDs are defined in hero-flex hierarchy state', () => {
  for (const id of [
    'SEAT_SHELL',
    'SEAT_CONNECTION',
    'SEAT_BEHAVIOR',
...
test('HierarchyRuntimeState fields and phases are present (R1)', () => {
  assert.match(runtime, /hierarchyRuntime/);
  assert.match(runtime, /openParentId/);
  assert.match(runtime, /focusedChildId/);
  assert.match(runtime, /focusedLeafId/);
  assert.match(runtime, /HIERARCHY_PHASE/);
  assert.match(runtime, /['"]rest['"]/);
  assert.match(runtime, /['"]opening['"]/);
  assert.match(runtime, /['"]open['"]/);
  assert.match(runtime, /['"]closing['"]/);
  assert.match(runtime, /NAVIGATE/);
  assert.match(runtime, /INSPECT/);
});

test('one-open closeHierarchyParent exists and is presentation-only', () => {
  assert.match(runtime, /function closeHierarchyParent/);
  assert.match(runtime, /getHierarchyState/);
  assert.match(runtime, /presentation only|Presentation only/i);
});

test('v1 children order excludes deferred Toolkit and ZipSkills', () => {
  assert.match(runtime, /SEAT_SHELL_V1_CHILDREN/);
  const childrenBlock = runtime.match(/SEAT_SHELL_V1_CHILDREN\s*=\s*\[[^\]]+\]/);
...
test('TeamAiHero exposes hierarchy getters without domain authority', () => {
  assert.match(runtime, /getHierarchyState/);
  assert.match(runtime, /closeHierarchyParent/);
  assert.doesNotMatch(runtime, /firestore|paypal|scheduler eligibility/i);
});
