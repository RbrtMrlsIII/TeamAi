...
test('draw paths use named scales at the canonical renderer boundary', () => {
  assert.match(hero, /RING_R0_ZIP_SCALE/);
  assert.match(renderer, /ringScale: RING_R1_SCALE/);
  assert.match(renderer, /ringScale: RING_R2_SCALE/);
  assert.match(r1, /workspaceRadius/);
  assert.match(r2, /workspaceRadius/);
  assert.doesNotMatch(renderer, /workspace \* (?:0\.22|1\.18|1\.42)/);
});
