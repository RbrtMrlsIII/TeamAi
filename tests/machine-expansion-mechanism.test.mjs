...
test('S5 full Seat-density expansion stays clear of all authored machine obstacles', () => {
  const divisions = [
    'SEAT_CONNECTION',
    'SEAT_BEHAVIOR',
    'SEAT_TOOLKIT',
  ...
  for (let seatCount = 1; seatCount <= 10; seatCount += 1) {
    for (const shellAmount of [0, 0.5, 1]) {
      const core = createBranchConnectionCore({
        seatCount,
        expansionAmount: shellAmount,
      });
      const seats = core.parts.filter((part) => part.kind === 'inner-pod');
      const obstaclesFor = (seat) => core.parts.filter((part) =>
        part.kind === 'outer-housing'
        || (part.kind === 'inner-pod' && part.branchId !== seat.branchId),
      );
      ...
      for (const seat of seats) {
        for (const [childIndex, childId] of divisions.entries()) {
          ...
          assert.equal(
            plan?.clearancePlan?.collision,
            false,
            `expansion collision: seats=${seatCount}, shell=${shellAmount}, ${seat.branchId}, ${childId}`,
          );
          assert.equal(
            plan?.clearancePlan?.maxSafeAmount,
            1,
            `expansion was limited: seats=${seatCount}, shell=${shellAmount}, ${seat.branchId}, ${childId}`,
          );
        }
      }
    }
