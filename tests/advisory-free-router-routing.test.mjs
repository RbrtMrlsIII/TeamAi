...
test('validation workflows avoid duplicate PR branch pushes',()=>{const fs2=require('node:fs');const full=fs2.readFileSync(path.join(root,'.github/workflows/full-system-verification.yml'),'utf8');const browser=fs2.readFileSync(path.join(root,'.github/workflows/playwright.yml'),'utf8');assert.match(full,/push:\n\s+branches: \[main\]/);assert.match(browser,/push:\n\s+branches: \[main\]/);assert.doesNotMatch(full,/governance\/repository-foundation/);assert.doesNotMatch(browser,/governance\/repository-foundation/);});
