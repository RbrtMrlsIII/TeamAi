...
test('automatic fan-out uses a two-second launch stagger from the canonical registry',()=>{assert.deepEqual(slots.map(s=>s.start_delay_seconds),[0,2,4,6,8]);assert.equal(manifest.advisory_review.automatic.max_spread_seconds,8);assert.match(sequence,/Resolved canonical advisory slot matrix/);assert.match(runner,/sleep \\\"\\\$START_DELAY_SECONDS\\\"/);});
