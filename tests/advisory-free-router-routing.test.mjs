...
test('runner accepts fully bolded Owning/Governing Issue fields',()=>{const issueLine=/^\s*(?:[-*]\s*)?(?:\*\*)?\s*(?:Owning|Governing)\s+Issue:\s*(?:\*\*)?#(\d+)/im;assert.equal('**Owning Issue: #394**'.match(issueLine)?.[1],'394');assert.equal('- **Governing Issue: #393**'.match(issueLine)?.[1],'393');assert.equal('Owning Issue: **#394**'.match(issueLine)?.[1],'394');assert.ok(controlPlaneValidator.includes('(?:\\*\\*)?\\s*(?:Owning|Governing)\\s+Issue:'));});
