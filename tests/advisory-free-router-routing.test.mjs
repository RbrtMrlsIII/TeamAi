...
test('manual advisory routing enforces the canonical five-slot registry shape',()=>{assert.match(manual,/len\(slots\) != 5/);assert.match(manual,/seen_slots=set\(\)/);assert.match(manual,/seen_aliases=set\(\)/);assert.match(manual,/\[1,2,3,4,5\]/);assert.match(manual,/\[0,2,4,6,8\]/);assert.match(manual,/not alias or not alias/);});
