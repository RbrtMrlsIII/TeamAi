...
test('canonical governance roots replace retired root files', () => {
  for (const p of [
    ...
    'Product_Law/CONNECTED_PLATFORM.md',
...
test('authority manifest is machine-readable and names one owner per canonical role', () => {
  ...
  assert.equal(manifest.active_authorities.connected_platform.path, 'Product_Law/CONNECTED_PLATFORM.md');
...
test('authority manifest records the validation migrations instead of deleting the old invariants silently', () => {
  ...
  for (const [old, expectedDisposition, replacement] of [
    ['root Product Law path', 'OBSOLETE', 'Product_Law/PRODUCT_LAW.md'],
    ['root Masterplan path', 'OBSOLETE', 'Masterplan/MASTERPLAN.md'],
    ['root current-slice path', 'OBSOLETE', 'Masterplan/NEXT_SLICES.md'],
    ['R1-R10 shared root assertion', 'OBSOLETE', 'R0-R3 ring map owns current topology; Skills do not own ring constants'],
    ['historical V3.x document as current frontier', 'OBSOLETE', 'Masterplan/NEXT_SLICES.md owns current frontier'],
    ['docs/PRODUCT_LAW_CONNECTED_PLATFORM_AUTHORITY.md as Product-Law parent', 'MOVED', 'Product_Law/CONNECTED_PLATFORM.md subordinate to Product_Law/PRODUCT_LAW.md'],
  ]) {
    assert.equal(disposition.get(old)?.disposition, expectedDisposition, old);
...
test('field wiring and Skill wiring remain separate responsibilities', () => {
  ...
  assert.match(skillWiring, /## Validation lifecycle routing/);
  assert.match(skillWiring, /docs\/skills/);
  assert.match(skillWiring, /forbidden as active routing/i);
...
test('promotion boundaries keep merged governance and machine candidate separate', () => {
  const manifest = JSON.parse(read('.github/teamai/authority-manifest.yml'));
  const skillWiring = read('docs/SKILL_WIRING.md');
  const session = read('AI_ASSISTANT_READ_ME.md');
